...
createAWindow title = do
    ...
    t <- getCurrentTime
    timeRef <- newIORef t
    -- saves position of player, and base Velocity
    playerPos <- newIORef (realToFrac 0, realToFrac 0)
    playerVel <- newIORef (realToFrac 0, realToFrac 0)
    -- set up ReactHandle
...
keyboardMouse playerVel key Down _ _ = case key of
    (SpecialKey KeyLeft) -> playerVel $~! \(x,y) -> ((-0.02),y)
    (SpecialKey KeyRight) -> playerVel $~! \(x,y) -> (0.02,y)
    (SpecialKey KeyUp) -> playerVel $~! \(x,y) -> (x,0.02)
    (SpecialKey KeyDown) -> playerVel $~! \(x,y) -> (x,(-0.02))
    _ -> return ()
